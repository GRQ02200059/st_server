...
class GameServerHandlerProtocolTest {
            ...
            expectedBodies.map { (commandId, expectedBody) ->
                Executable {
                    ...
                    try {
                        ...
                        when (commandId) {
                            708, 2_670, 2_679, 2_683, 2_698, 3_845 -> {
    ...
    @Test
    fun `invitational team log accepts only exact fixed tuple and remains repository free`() {
        val commandId = 3_519
        val accountKey = "invitational-team-log-boundary"
        val state = PlayerStateRepository.getOrCreate(
            accountKey = accountKey,
            cityWid = GameServerConfig.CITY_WID,
            roleName = "Invitational Team Log User",
        )
        state.resources.money = 3_519_000
        state.addHero(heroId = 100_101, nowSec = 1_700_000_000)
        UnionStateRepository.create(state, "Invitational Team Log Union", nowSec = 8)
        assertTrue(WorldStateRepository.claimLand(state, wid = 10_032, nowSec = 8))
        PlayerStateRepository.save(state)
        val playerBefore = state.toSnapshot()
        val persistedPath = repositoryRoot.resolve("accounts").resolve("$accountKey.json")
        val persistedBytesBefore = Files.readAllBytes(persistedPath)
        ...
        val invalidRequests = linkedMapOf(
            ...
            "empty text" to "",
            "malformed JSON" to "[",
            "number scalar" to "3519",
            "string scalar" to """"synthetic-invitational-log-canary"""",
            "boolean scalar" to "true",
            "top-level object" to """{"synthetic":"invitational-log-canary"}""",
            "empty array" to "[]",
            "one-slot array" to "[0]",
            "nested first slot" to "[[],20000]",
            "nested second slot" to "[0,[]]",
        ...
        try {
            ...
            assertEquals(commandId, packet.cmd)
            assertNotEquals(Cmd.SYS_NOTIFY_DB_UPDATE, packet.cmd)
            assertNotEquals(3_518, packet.cmd)
            assertNotEquals(3_520, packet.cmd)
            assertEquals(DownType.PLAIN, packet.dataType)
            assertTrue(
                "[]".toByteArray().contentEquals(packet.body),
                "valid [0,20000] request emitted wrong wire bytes",
            ...
            assertNull(
                channel.readOutbound<Any>(),
                "valid request emitted an unsolicited, cmd 90005, adjacent, or extra packet",
            )
            assertStateUnchanged("valid request")
        } finally {
            channel.finishAndReleaseAll()
    ...
    @Test
    fun `family log accepts only exact fixed tuple and remains repository free`() {
        val commandId = 3_908
        val accountKey = "family-log-boundary"
        val state = PlayerStateRepository.getOrCreate(
            accountKey = accountKey,
            cityWid = GameServerConfig.CITY_WID,
            roleName = "Family Log User",
        )
        state.resources.money = 3_908_000
        state.addHero(heroId = 100_101, nowSec = 1_700_000_000)
        UnionStateRepository.create(state, "Family Log Union", nowSec = 9)
        assertTrue(WorldStateRepository.claimLand(state, wid = 10_033, nowSec = 9))
        PlayerStateRepository.save(state)
        val playerBefore = state.toSnapshot()
        val persistedPath = repositoryRoot.resolve("accounts").resolve("$accountKey.json")
        val persistedBytesBefore = Files.readAllBytes(persistedPath)
        ...
        val invalidRequests = linkedMapOf(
            ...
            "empty text" to "",
            "malformed JSON" to "[",
            "number scalar" to "3908",
            "string scalar" to """"synthetic-family-log-canary"""",
            "boolean scalar" to "true",
            "top-level object" to """{"synthetic":"family-log-canary"}""",
            "empty array" to "[]",
            "one-slot array" to "[0]",
            "nested first slot" to "[[],20000]",
            "nested second slot" to "[0,[]]",
        ...
        try {
            ...
            assertEquals(commandId, packet.cmd)
            assertNotEquals(Cmd.SYS_NOTIFY_DB_UPDATE, packet.cmd)
            assertNotEquals(3_907, packet.cmd)
            assertNotEquals(3_909, packet.cmd)
            assertEquals(DownType.PLAIN, packet.dataType)
            assertTrue(
                "[]".toByteArray().contentEquals(packet.body),
                "valid [0,20000] request emitted wrong wire bytes",
    ...
    @Test
    fun `military strategy union log accepts only empty array and remains repository free`() {
        val commandId = 3_635
        val accountKey = "military-strategy-union-log-boundary"
        val state = PlayerStateRepository.getOrCreate(
            accountKey = accountKey,
            cityWid = GameServerConfig.CITY_WID,
            roleName = "Military Strategy Union Log User",
        )
        state.resources.money = 3_635_000
        state.addHero(heroId = 100_101, nowSec = 1_700_000_000)
        UnionStateRepository.create(state, "Military Strategy Log Union", nowSec = 10)
        assertTrue(WorldStateRepository.claimLand(state, wid = 10_034, nowSec = 10))
        PlayerStateRepository.save(state)
        val playerBefore = state.toSnapshot()
        val persistedPath = repositoryRoot.resolve("accounts").resolve("$accountKey.json")
        val persistedBytesBefore = Files.readAllBytes(persistedPath)
        ...
        val invalidRequests = linkedMapOf(
            ...
            "empty text" to "",
            "malformed JSON" to "[",
            "number scalar" to "3635",
            "string scalar" to """"synthetic-strategy-log-canary"""",
            "boolean scalar" to "true",
            "top-level object" to """{"synthetic":"strategy-log-canary"}""",
            "one-slot array" to "[0]",
            "string array" to """["synthetic-strategy-log-canary"]""",
            "nested array" to "[[]]",
            "two-slot array" to "[0,20000]",
            "trailing token" to "[] {}",
        )
        ...
        try {
            ...
            assertEquals(commandId, packet.cmd)
            assertNotEquals(Cmd.SYS_NOTIFY_DB_UPDATE, packet.cmd)
            assertNotEquals(180, packet.cmd)
            assertNotEquals(6_069, packet.cmd)
            assertEquals(DownType.PLAIN, packet.dataType)
            assertTrue(
                "[]".toByteArray().contentEquals(packet.body),
            ...
            assertNull(
                channel.readOutbound<Any>(),
                "valid request emitted an unsolicited, cmd 90005, paired, or extra packet",
            )
            assertStateUnchanged("valid request")
        } finally {
            channel.finishAndReleaseAll()
