...
class CommandContractRegistryTest {
    ...
    @Test
    fun `union plan chat history exposes exact constant and handler owned social client request contract`() {
        val name = "CHAT_UNION_PLAN_HISTORY"
        val commandId = 6_054
        val field = assertNotNull(
            runCatching { Cmd::class.java.getField(name) }.getOrNull(),
            "missing Cmd.$name",
    ...
    @Test
    fun `family log exposes exact constant and handler owned social client request contract`() {
        val name = "FAMILY_LOG_GET"
        val commandId = 3_908
        val field = assertNotNull(
            runCatching { Cmd::class.java.getField(name) }.getOrNull(),
            "missing Cmd.$name",
    ...
    @Test
    fun `military strategy union log exposes exact constant and handler owned social duplex contract`() {
        val name = "GET_MILITARY_STRATEGY_UNION_LOG"
        val commandId = 3_635
        val field = assertNotNull(
            runCatching { Cmd::class.java.getField(name) }.getOrNull(),
            "missing Cmd.$name",
