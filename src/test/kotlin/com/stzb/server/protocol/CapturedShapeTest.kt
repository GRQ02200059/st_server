...
class CapturedShapeTest {
    ...
    @Test
    fun `union plan chat history stays outside captured shape fallback`() {
        val commandId = 6_054
        ...
        assertNull(
            NetworkResponsePolicy.observedShapeBody(
                commandId,
                "[17] synthetic-union-plan-chat-canary",
            ),
        )
        assertTrue(commandId !in NetworkResponsePolicy.observedShapeCommandIds())
    }
    ...
    @Test
    fun `handler owned invitational team log is absent from observed shape fallback`() {
        val commandId = 3_519
    ...
    @Test
    fun `handler owned family log is absent from observed shape fallback`() {
        val commandId = 3_908
        assertNull(NetworkResponsePolicy.observedShapeBody(commandId, "[0,20000]"))
        assertTrue(commandId !in NetworkResponsePolicy.observedShapeCommandIds())
    }
    ...
    @Test
    fun `handler owned military strategy union log is absent from observed shape fallback`() {
        val commandId = 3_635
