...
- 词条优先选相同 `gear_type` 的最高等级鸿级词条 (`advance=1`)；
  若该类型没有鸿级词条，使用当前配置中最高等级鸿级词条。
...
当前干净 9.2.2 客户端的实际 `tb_cfg_gear_feature.bin` 只有普通
`level_type=0/2`，不包含原生红档 `30`。全鸿级规则使用客户端真实的
`advance=1` 顶级词条，因此不依赖色彩增强设置或客户端注入。
...
1. 加入 `Tb_gear` 的全部基础武器鸿级行和 50 条鸿级副本。
