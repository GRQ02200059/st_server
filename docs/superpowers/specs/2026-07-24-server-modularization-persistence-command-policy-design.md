...
- 每个业务模块通过小接口封装请求解析、状态修改、持久化和响应生成。
...
- `cmd=32` 使用请求中的两个 `armyId`，允许同队换位和跨队交换。
- `cmd=37/38` 如果请求未直接携带 `armyId`，通过请求涉及的武将反查所属部队。
  所有武将必须唯一指向同一部队；无法定位或涉及多支部队时返回安全失败，不猜测默认队伍。
...
1. 先建立命令清单和路由测试，用现有 handler 行为作为基线。
2. 引入模块接口，逐领域迁移 handler 方法，保持响应字节行为。
3. 将单队伍模型升级为多城多队，并修正所有快照与通知生成。
4. 抽取玩家仓库接口，保留内存 adapter。
5. 引入 SQLite schema、migration 和运行时 adapter。
6. 将状态修改命令切换为事务更新，补重启恢复测试。
7. 删除旧的宽泛业务空数组兜底，完成全量回归和客户端联调。
